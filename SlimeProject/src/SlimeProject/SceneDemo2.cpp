...
void SceneDemo2::OnInitialize()
{
	...
	AssetManager::GetMusic("odyssey").setVolume(15);
	...
	// Create Entity 1 = player
	{
		...
		//mp_player->Init(mp_gameManager, mp_gameManager->GetEntityManager(), this);
	...
	// === CHEMIN FACILE (a gauche) (chemin + orientEsur l'absorption et la regurgitation d'item peut etre)
	// === Pont de sortie ===
...
void SceneDemo2::OnUpdate()
{
	...
	{
		if (playerComponent->m_inventory.empty() == false)
		{
			InventoryItem lastItem;
			lastItem = playerComponent->m_inventory.top();

			if (lastItem.m_textureID == L"DroneTexture")
			{
				CreateEnnemy(lastItem.vScale.x, lastItem.vScale.y, lastItem.vScale.z, transform->m_transform.vPosition.x, transform->m_transform.vPosition.y, transform->m_transform.vPosition.z + transform->m_transform.vScale.z);
				playerComponent->m_inventory.pop();
			}
			test++;
...
void SceneDemo2::OnClose()
{
	Close();
}

void SceneDemo2::CreateBoxBlock(float sizeX, float sizeY, float sizeZ, float posX, float posY, float posZ, int health, float intensity)
{
...
void SceneDemo2::CreateFloor(float sizeX, float sizeY, float sizeZ, float posX, float posY, float posZ)
{
	...
	for (auto& comp : mpEntityManager->GetComponentToAddTab()[floor->tab_index]->vec_components)
	{
		if (comp->ID == Mesh_ID)
		{
			MeshComponent* mesh = static_cast<MeshComponent*>(comp);
			mesh->m_mesh = mp_gameManager->GetFactory()->CreateCube();
			mesh->m_textureID = L"TileTexture";
		}
		if (comp->ID == Transform_ID)
		{
			TransformComponent* transform = static_cast<TransformComponent*>(comp);
...
void SceneDemo2::CreateWall(float sizeX, float sizeY, float sizeZ, float posX, float posY, float posZ)
{
...
void SceneDemo2::CreateStickyWall(float sizeX, float sizeY, float sizeZ, float posX, float posY, float posZ)
{
...
void SceneDemo2::CreateEnnemy(float sizeX, float sizeY, float sizeZ, float posX, float posY, float posZ)
{
...
void SceneDemo2::CreateProjectile(float sizeX, float sizeY, float sizeZ, float posX, float posY, float posZ)
{
...
void SceneDemo2::CreateSphere(float sizeX, float sizeY, float sizeZ, float posX, float posY, float posZ)
{
...
void SceneDemo2::CreateTree1(float scale, float posX, float posY, float posZ)
{
	// === Test arbre === 
